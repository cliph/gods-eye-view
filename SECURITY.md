...
- **App-level throttles:** `GEV_RATELIMIT_OPENAI_PER_MIN` and `GEV_RATELIMIT_GOOGLE_PER_MIN` are opt-in; `GEV_RATELIMIT_CCTV_STREETVIEW_PER_MIN` is **on by default at 240/min** (set it to 0 to opt out) because the app polls that endpoint itself rather than waiting for a user action. They cap the cost-bearing endpoints per client IP per minute (over-limit requests receive a sanitized `429`, except the CCTV Street View fallback, which degrades to its synthetic camera frame so the grid keeps rendering). They are **per-IP, process-local, in-memory guards** — they reset on restart and are **not billing caps**. The CCTV Street View fallback additionally refuses any frame request whose camera id is not in the server catalog — including when the catalog is empty, since that signals a failed source load rather than a deliberately bare install — and any pose further than 1.5 km from that camera's registered position, so `/api/cctv/frame/:id` cannot be driven as a general-purpose Street View proxy.
